...
def mse(dim, paddings=None, weight_fn=None, apply_idct=False, dct_min=None, dct_max=None):
    def masked_mse(y_true, y_pred):
        weight_tensor = tf.ones_like(y_true) if weight_fn is None else weight_fn(y_true)
        if paddings is not None:
            temp = tf.constant(value=1.0, shape=dim)
            mask = tf.pad(temp, paddings=paddings)
            y_true = mask * y_true
        ...
        if apply_idct:
            ...
            y_true = tf.cast((tf.cast(y_true, dtype=tf.float16) * dct_diff) + min_tf, dtype=tf.float32)
            y_pred = tf.cast((tf.cast(y_pred, dtype=tf.float16) * dct_diff) + min_tf, dtype=tf.float32)
            se_val = tf.math.squared_difference(image_batch_idct(y_true), image_batch_idct(y_pred))
        else:
            se_val = tf.math.squared_difference(y_pred, y_true)

        return tf.math.reduce_mean(weight_tensor*se_val, axis=-1)

    return masked_mse


def mae(dim, paddings=None, weight=1, apply_idct=False, dct_min=None, dct_max=None):
    def masked_mae(y_true, y_pred):
        ...
        if apply_idct:
            ...
            y_true = tf.cast((tf.cast(y_true, dtype=tf.float16) * dct_diff) + min_tf, dtype=tf.float32)
            y_pred = tf.cast((tf.cast(y_pred, dtype=tf.float16) * dct_diff) + min_tf, dtype=tf.float32)
            mae_val = tf.metrics.mean_absolute_error(image_batch_idct(y_true), image_batch_idct(y_pred))
        else:
            mae_val = tf.metrics.mean_absolute_error(y_pred, y_true)

        return weight * mae_val
...
def dct_and_pixelwise_mse(dim, paddings=None, mse_weight=1., dct_weight=1.):
    def masked_dct_and_mse(y_true, y_pred):
        ...
        dct_mse_val = tf.reduce_mean(dct_mse_val, axis=(-1, -2))
        mse_val = tf.metrics.mean_squared_error(y_true, y_pred)
        mse_val = tf.reduce_mean(mse_val, axis=(-1, -2))
...
def dct_and_pixelwise_mae(dim, paddings=None, mse_weight=1., dct_weight=1.):
    def masked_dct_and_mae(y_true, y_pred):
        if paddings is not None:
            temp = tf.constant(value=1.0, shape=dim)
            mask = tf.pad(temp, paddings=paddings)
            y_true = mask * y_true
        ...
        dct_mse_val = tf.reduce_mean(dct_mse_val, axis=(-1, -2))
        mse_val = tf.metrics.mean_absolute_error(y_true, y_pred)
        mse_val = tf.reduce_mean(mse_val, axis=(-1, -2))
        return (mse_weight * mse_val) + (dct_weight * dct_mse_val)

    return masked_dct_and_mae
...
def multiscale_ssim_l2(dim, weight_fn=None, paddings=None):
    def masked_mssim_l2(y_true, y_pred):
        ...
        wl2 = weight_tensor*tf.math.squared_difference(y_true, y_pred)
        loss = tf.math.reduce_mean(ms_ssim) + tf.math.reduce_mean(wl2)
        return loss

    return masked_mssim_l2


def ssim_l2(dim, weight_fn=None, paddings=None):
    def masked_ssim_l2(y_true, y_pred):
        weight_tensor = tf.ones_like(y_true) if weight_fn is None else weight_fn(y_true)
        if paddings is not None:
            temp = tf.constant(value=1.0, shape=dim)
            mask = tf.pad(temp, paddings=paddings)
        ...
        wl2 = weight_tensor*tf.math.squared_difference(y_true, y_pred)
        loss = tf.math.reduce_mean(ssim) + tf.math.reduce_mean(wl2)
        return loss

    return masked_ssim_l2
