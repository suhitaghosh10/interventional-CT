...
# take the difference between current pixel  and previous pixels
def baur_difference(input, mode=None, constant_values=None):
    """
    Apply baur difference filter to image.
    Args:
      input: A 4-D (`[N, H, W, C]`) Tensor.
      ksize: A scalar Tensor. Kernel size.
      mode: A `string`. One of "CONSTANT", "REFLECT", or "SYMMETRIC"
    ...
    g = tf.convert_to_tensor(np.array([[ 0, 0,  0],
                                        [ -1, 2,  0],
