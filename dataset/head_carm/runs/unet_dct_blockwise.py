...
from utility.utils import ssim, psnr, mse, dct_mse, blockwisedct_and_pixelwise_mse, blockwisedct_mse
...
NAME = 'Unet_Prior_needle_blockwiseDCT_MSE'+ '_D' + str(d) + 'Lr' + str(lr)+ '_S'+str(SPARSE_PROJECTION_NUM)
...
def run_training():
   ...
   # optimizer = AdamWithWeightnorm(learning_rate=lr)
    ...
    model.compile(optimizer=optimizer,
                  ...
                  loss=blockwisedct_and_pixelwise_mse(IMG_DIM_INP_2D),
                  metrics=[mse(IMG_DIM_INP_2D),
                           ssim(IMG_DIM_INP_2D),
                           psnr(IMG_DIM_INP_2D),
                           blockwisedct_mse(IMG_DIM_INP_2D)
                           ])
    model.summary()
