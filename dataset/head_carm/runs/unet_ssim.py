...
from utility.utils import ssim, psnr, mse, mssim, ssim_l2
...
NAME = 'Unet_Prior_needle_SSIM'+ '_D' + str(d) + 'Lr' + str(lr)+ '_S'+str(SPARSE_PROJECTION_NUM)
...
def run_training():
   ...
   # optimizer = AdamWithWeightnorm(learning_rate=lr)
    ...
    model.compile(optimizer=optimizer,
                  ...
                  loss=ssim_l2(IMG_DIM_INP_2D, mse_weight=1., ssim_weight=1.),
    ...
    es = tfk.callbacks.EarlyStopping(monitor=save_by, mode='max', verbose=1, patience=20, min_delta=1e-2)
