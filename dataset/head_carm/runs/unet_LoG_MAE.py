...
from utility.utils import ssim, psnr, mse, mssim, laplacian_of_gaussian_MAE, laplacian_of_gaussian_mae_metric
...
NAME = 'Unet_Prior_needle_LoG_MAE_v2'+ '_D' + str(d) + 'Lr' + str(lr)+ '_S'+str(SPARSE_PROJECTION_NUM)
...
def run_training():
   ...
   # optimizer = AdamWithWeightnorm(learning_rate=lr)
    model.compile(optimizer=optimizer,
                  ...
                  loss=laplacian_of_gaussian_MAE(IMG_DIM_INP_2D),
                  metrics=[mse(IMG_DIM_INP_2D),
                           mssim(IMG_DIM_INP_2D),
                           ssim(IMG_DIM_INP_2D),
                           psnr(IMG_DIM_INP_2D),
                           laplacian_of_gaussian_mae_metric(IMG_DIM_INP_2D)
                           ])
    model.summary()
