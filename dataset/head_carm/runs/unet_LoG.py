...
from utility.utils import ssim, psnr, mse, mssim, laplacian_of_gaussian_MSE, laplacian_of_gaussian_mse_metric
...
NAME = 'Unet_Prior_needle_LoG0.1_MSE10.0'+ '_D' + str(d) + 'Lr' + str(lr)+ '_S'+str(SPARSE_PROJECTION_NUM)
...
def run_training():
   ...
   # optimizer = AdamWithWeightnorm(learning_rate=lr)
    model.compile(optimizer=optimizer,
                  ...
                  loss=laplacian_of_gaussian_MSE(IMG_DIM_INP_2D, mse_weight=10., log_weight=0.1),
                  metrics=[mse(IMG_DIM_INP_2D),
                           mssim(IMG_DIM_INP_2D),
                           ssim(IMG_DIM_INP_2D),
                           psnr(IMG_DIM_INP_2D),
                           laplacian_of_gaussian_mse_metric(IMG_DIM_INP_2D, weight=.1)
                           ])
    model.summary()
